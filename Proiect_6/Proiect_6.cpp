...
public:
    ...
    friend ofstream &operator<<(ofstream &file, const Cafea &c)
    {
        file << c.origine << " " << c.codProdus << " " << c.getScorIntensitate() << endl;
        return file;
    }

    friend ifstream &operator>>(ifstream &file, Cafea &c)
    {
        file >> c.origine >> c.codProdus;
        float scor;
        file >> scor;
        if (c.scorIntensitate != nullptr)
        {
            *c.scorIntensitate = scor;
        }
        else
        {
            c.scorIntensitate = new float(scor);
        }
        return file;
    }
...
public:
    ...
    // Operatori pt. fisiere TEXT
    friend ofstream &operator<<(ofstream &file, const Angajat &a)
    {
        double ore = (a.orePeSaptamana != nullptr) ? *a.orePeSaptamana : 0.0;
        file << a.nume << " " << a.idAngajat << " " << ore << endl;
        return file;
    }

    friend ifstream &operator>>(ifstream &file, Angajat &a)
    {
        file >> a.nume >> a.idAngajat;
        double ore;
        file >> ore;
        if (a.orePeSaptamana != nullptr)
        {
            *a.orePeSaptamana = ore;
        }
        else
        {
            a.orePeSaptamana = new double(ore);
        }
        return file;
    }
};
...
public:
    ...
    {
        int lungimeMarca = marca.length();
        f.write((char *)&lungimeMarca, sizeof(int));
        for (int i = 0; i < lungimeMarca; i++)
        {
            f.write((char *)&marca[i], sizeof(char));
        }
        f.write((char *)&tensiuneStandard, sizeof(int));
        int cicluri = (cicluriService != nullptr) ? *cicluriService : 0;
        f.write((char *)&cicluri, sizeof(int));
    }

    void citesteDinFisierBinar(fstream &f)
    {
        int lungimeMarca;
        f.read((char *)&lungimeMarca, sizeof(int));
        marca = "";
        for (int i = 0; i < lungimeMarca; i++)
        {
            char c;
        ...
        f.read((char *)&tensiuneStandard, sizeof(int));
        int cicluri;
        f.read((char *)&cicluri, sizeof(int));
        if (cicluriService != nullptr)
        {
            *cicluriService = cicluri;
        }
        else
        {
            cicluriService = new int(cicluri);
...
public:
    ...
    // Metode py. fisiere BINARE
    void scrieInFisierBinar(fstream &f)
    {
        f.write((char *)&nrAngajati, sizeof(int));

        int lungimeAdresa = (adresaCafenea != nullptr) ? strlen(adresaCafenea) : 0;
        f.write((char *)&lungimeAdresa, sizeof(int));
        for (int i = 0; i < lungimeAdresa; i++)
        {
            f.write((char *)&adresaCafenea[i], sizeof(char));
        }

        int lungimeNume = barista.getNume().length();
        f.write((char *)&lungimeNume, sizeof(int));
        string numeBarista = barista.getNume();
        for (int i = 0; i < lungimeNume; i++)
        {
            f.write((char *)&numeBarista[i], sizeof(char));
        }
        int idBarista = barista.getIdAngajat();
        f.write((char *)&idBarista, sizeof(int));
        double oreBarista = (barista.getOrePeSaptamana() != nullptr) ? *barista.getOrePeSaptamana() : 0.0;
        f.write((char *)&oreBarista, sizeof(double));
    }
    ...
    void citesteDinFisierBinar(fstream &f)
    {
        ...
        int lungimeAdresa;
        f.read((char *)&lungimeAdresa, sizeof(int));
        if (adresaCafenea != nullptr)
        {
            delete[] adresaCafenea;
        }
        if (lungimeAdresa > 0)
        {
            adresaCafenea = new char[lungimeAdresa + 1];
            for (int i = 0; i < lungimeAdresa; i++)
            {
                f.read((char *)&adresaCafenea[i], sizeof(char));
            }
            adresaCafenea[lungimeAdresa] = '\0';
        }
        else
        {
            adresaCafenea = nullptr;
        ...
        barista.setNume(numeBarista);
        int idBarista;
        f.read((char *)&idBarista, sizeof(int));
        barista.setIdAngajat(idBarista);
        double oreBarista;
        f.read((char *)&oreBarista, sizeof(double));
        barista.setOrePeSaptamana(oreBarista);
    }
};
...
{
    // == FISIERE TEXT ==
    cout << "=== Test fisiere TEXT ===" << endl;

    // Scriere Cafea in fisier text
    Cafea cafea1("Columbia", 101, 9.5f);
    ofstream fisierCafeaOut("cafea.txt", ios::out);
    fisierCafeaOut << cafea1;
    fisierCafeaOut.close();
    cout << "Cafea scrisa in fisier text." << endl;

    // Citire  Cafea din fisier
    Cafea cafeaCitita;
    ifstream fisierCafeaIn("cafea.txt", ios::in);
    fisierCafeaIn >> cafeaCitita;
    fisierCafeaIn.close();
    cout << "Cafea citita din fisier: " << cafeaCitita << endl;

    Angajat angajat1("Mihai", 501, 45.0);
    ofstream fisierAngajatOut("angajat.txt", ios::out);
    fisierAngajatOut << angajat1;
    fisierAngajatOut.close();
    cout << "Angajat scris in fisier text." << endl;

    Angajat angajatCitit;
    ifstream fisierAngajatIn("angajat.txt", ios::in);
    fisierAngajatIn >> angajatCitit;
    fisierAngajatIn.close();
    cout << "Angajat citit din fisier: " << angajatCitit << endl;

    // ===== FISIERE BINARE =====
    cout << "\n=== Test fisiere BINARE ===" << endl;

    Espressor espressor1("DeLonghi", 230, 150);
    fstream fisierEspressorOut("espressor.bin", ios::out | ios::binary);
    espressor1.scrieInFisierBinar(fisierEspressorOut);
    fisierEspressorOut.close();
    cout << "Espressor scris in fisier binar." << endl;

    Espressor espressorCitit;
    fstream fisierEspressorIn("espressor.bin", ios::in | ios::binary);
    espressorCitit.citesteDinFisierBinar(fisierEspressorIn);
    fisierEspressorIn.close();
    cout << "Espressor citit din fisier: " << espressorCitit << endl;

    Angajat barista("Ana", 601, 40.0);
    Cafenea cafenea1(5, "Str. Victoriei 10", barista);
    fstream fisierCafeneaOut("cafenea.bin", ios::out | ios::binary);
    cafenea1.scrieInFisierBinar(fisierCafeneaOut);
    fisierCafeneaOut.close();
    cout << "Cafenea scrisa in fisier binar." << endl;

    Cafenea cafeneaCitita;
    fstream fisierCafeneaIn("cafenea.bin", ios::in | ios::binary);
    cafeneaCitita.citesteDinFisierBinar(fisierCafeneaIn);
    fisierCafeneaIn.close();
    cout << "Cafenea citita din fisier: " << cafeneaCitita << endl;
